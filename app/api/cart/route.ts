...
export async function POST(req:Request){
    try {
        ...
        for(const product of products ){
            ...
            const existingCart = await Cart.findOne({ where: {productId: productId, userId: user.id}}) as CartModel

            if (!existingCart) {
                 await Cart.create({ userId: user.id, productId, quantity: count, price })
            } else {
